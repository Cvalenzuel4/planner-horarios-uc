...
export const ConflictDetailModal: React.FC<ConflictDetailModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            {/* Modal */}
            <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden border border-gray-200 dark:border-gray-800">
                {/* Header */}
                ...
                <div className="px-6 py-5">
                        ...
                        {bloquesEnConflicto.map((bloque, index) => {
                            ...
                            return (
                                <Fragment key={`${bloque.seccion.id}-${index}`}>
                                    ...
                                    <div
                                        className={`rounded-xl border-2 ${colores.border} overflow-hidden transition-all hover:scale-[1.01] bg-white dark:bg-gray-800`}
                                    >
                                        {/* Ramo header */}
                                        <div className={`px-4 py-3 ${colores.bg}`}>
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className={`${colores.text} text-sm opacity-80 px-2 py-0.5 rounded-full bg-white/50 dark:bg-black/20`}>
                                                        Sección {bloque.seccion.numero}
                                                    </span>
                                                </div>
                                                <span className={`text-xs px-2 py-1 rounded-full ${colores.text} bg-white/50 dark:bg-black/20 font-medium`}>
                    ...
                    {/* Summary */}
                    <div className="mt-5 p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50">
                ...
                {/* Footer */}
                <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex justify-end bg-gray-50/50 dark:bg-gray-800/30">
                    <button
                        onClick={onClose}
